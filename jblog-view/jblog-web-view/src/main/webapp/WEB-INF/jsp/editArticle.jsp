...
<%-- 
  ...
  - Description: This page displays a form of editing articles.
  --%>
    ...
    <div class="container">
        ...
        <div class="content">
            <c:if test="${not empty message}">
                <p><span style="color: green;"><fmt:message key="${message}" /></span></p>
            </c:if>
            ...
            <form action="${pageContext.request.contextPath}/edit-article?articleId=${article.id}" method="post" name="addStoryForm">
                <p>
                    ...
                    <input type="submit" value="<fmt:message key="edit_article.button.update_article" />" id="submit" name="updateArticleButton" onclick="return validateStory()"><br><br>
                </p>
                <h3 align="center">${article.title}</h3>
                <p>
                    <c:out value="${article.body}" escapeXml="false"/><br>
                    <b><fmt:message key="display_article.label.keywords" />:</b>
                    <jblog:printKeywords keywords="${article.keywords}"/><br>
                </p>
            </form>
        </div>
